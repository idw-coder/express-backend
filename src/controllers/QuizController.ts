...
export class QuizController {
  ...
  async getQuizDetail(req: Request, res: Response): Promise<void> {
    try {
       ...
       */
      const taggings = await taggingRepo.find({
        where: { quizId },
        relations: ["quizTag"],
      });
  ...
  async createQuiz(req: Request, res: Response): Promise<void> {
    try {
      ...
      const { category_id, slug, question, explanation, choices, tags } =
        req.body as {
          category_id?: number;
          slug?: string;
          question?: string;
      ...
      if (!category_id || !slug || !question) {
        res
          .status(400)
          .json({ error: "category_id, slug, question are required" });
        return;
      }
      ...
      if (!saved) {
        res
          .status(201)
          .json({ id: quiz.id, slug: quiz.slug, message: "Created" });
        return;
      }
